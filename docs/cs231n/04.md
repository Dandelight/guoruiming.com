...
如果loss非常小（1e-10以下），可以考虑将loss放大一点。

#### Kinks

Kinks指目标函数中不可微的部分，此时的数值微分不是准确的。
...
Adam论文中推荐的参数设置是 `eps = 1e-8`, `beta1 = 0.9`, `beta2 = 0.999`。学习率设置在`5e-3`（从头训练）到`1e-5`（迁移学习）

一般用Adam，有时Adagrad>Adam (任务不同)
